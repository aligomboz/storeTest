                    ...
                    <tbody>
                        @foreach ($order->transactions as $transaction)
                            <tr>
                                ...
                                <td>
                                    @if ($loop->last && $transaction->transaction == \App\Models\OrderTransaction::FINISHED && \Carbon\Carbon::now()->addDays(5)->diffInDays($transaction->created_at->format('Y-m-d')) != 0)
                                        <button type="button" wire:click="requestReturnOrder('{{ $order->id }}')"
                                            class="btn btn-link text-right"> {{ __('you can return order in') }} {{ 5 - $transaction->created_at->diffInDays() }} {{ __('days') }}
                                        </button>
                                    @endif
                                </td>
                            </tr>
